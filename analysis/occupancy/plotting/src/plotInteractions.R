...
plotAllInteractions <- function(){
    ...
    for(i in 1:length(probs)){
        curve(inv.logit(means['mu.phi.0'] +
                        ...
                        means['mu.phi.fra'] * x +
                        means['mu.phi.hr.area.fra'] * x * quantiles$HRarea[i]),
    ...
    for(i in 1:length(probs)){
        curve(inv.logit(means['mu.phi.0'] +
                        means['mu.phi.nat.area'] * quantiles$natural[i] +
                        means['mu.phi.fra'] * x +
                        means['mu.phi.nat.area.fra'] * x * quantiles$natural[i]),
              from=range(model.input$data$fra)[1],
              to=range(model.input$data$fra)[2],
              col=cols[i],
              lwd=2,
    ...
    legend("topright", legend="Non-crop habitat proximity (b)", bty="n", cex=1.2)
