compareMCMCs_withMonitors <- function(modelInfo, MCMCs = c('nimble'),
                                      MCMCdefs,
                                      monitors, BUGSdir, stanDir,
                                      stanInfo,
                                      doSamplePlots = FALSE, verbose = TRUE, summary = TRUE, ...) {

    ## stanInfo = list(codeFile = required, dir = optional,
    ## stanParameterRules = optional, modelName = optional, data =
    ## optional, inits = optional) or simply a character codeFile
    ...
    ## At this point models is a character vector of model names and
    ## modelContents is a list with information about each model
    ...
    for (i in 1:length(models)){
        ...
        if(useStan) {
            if(!useStanInfo) {
                ## no stanInfo so use the model name for the
                ## stanModelName and stanCodefile
...
MCMCsuite_mod <- function(
    ...
    suite <- MCMCsuite_modClass(code, constants, data, inits, monitors, niter, burnin, thin, summaryStats, calculateEfficiency,
